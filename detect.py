...
def predict(img_path, weights,imgsz=320,conf=0.1, device='cpu'):
    """
    Predicts bounding boxes of the given image.
    ...
    imgsz : int, optional
        The size of the image to be predicted, by default 320.
    conf : float, optional
        The confidence threshold, by default 0.1.
    device : str, optional
        The device to use for prediction, by default 'cpu'.
    
    Returns
    -------
    list
        A list of cropped images, each corresponding to a bounding box's coordinates in the format (x1, y1, x2, y2).
    """
    bboxes = run(weights=weights, imgsz=imgsz, conf_thres=conf, source=img_path, device=device)
